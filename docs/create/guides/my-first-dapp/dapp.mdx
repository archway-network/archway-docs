...
To connect an end user to Archway Network, we’ll need our dApp to connect to users’ wallets using a browser extension like [Keplr](https://chrome.google.com/webstore/detail/keplr/dmkamcknogkgcdfhhbddcghachkejeap) wallet.
...
To connect to your dApp, Keplr needs to know some metadata about the chain it’s connecting to, and the type of contracts stored there.
Here's an example of connecting to Archway’s Constantine testnet, which supports CosmWasm smart contracts.
...
With that out of the way, we can use ChainInfo to set up Keplr for Constantine testnet.
...
To broadcast transactions we call the `execute` function, from our previous `CosmWasmClient` reference, using the following arguments:
