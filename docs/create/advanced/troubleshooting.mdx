...
By default projects created with the `archway new` command use a Docker Hub version of the `cosmwasm/rust-optimizer`. It would also be possible your own Docker image to use for this step. See the [Dockerfile](https://github.com/CosmWasm/rust-optimizer/blob/main/Dockerfile) and [Makefile](https://github.com/CosmWasm/rust-optimizer/blob/main/Makefile) for information about build parameters and dependencies.
...
If you installed `archwayd` locally from GitHub, you'll need to pull the latest changes reinstall it.
...
The above error isn't caused by your code. It seems to occur during periods of high network activity. We're tracking it in [this](https://github.com/archway-network/archway/issues/157) GitHub thread. 

We're actively looking for ways to solve it, but in the meantime your best bet is to continue broadcasting the transaction until it works.
