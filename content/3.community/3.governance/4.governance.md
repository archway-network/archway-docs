...
`archwayd q gov params --node https://rpc.constantine.archway.tech:443`
...
1. The `chain-id` for Constantine is `constantine-3`
...
`archwayd tx gov submit-proposal --from mywallet5 --node https://rpc.constantine.archway.tech:443 --chain-id constantine-3 --fees 
500uconst 
...
--node 
https://rpc.constantine.archway.tech:443 --chain-id constantine-3 --fees 500uconst`
...
`archwayd tx gov  submit-proposal cancel-software-upgrade --node https://rpc.constantine.archway.tech:443 --title "test" 
--description 
"test2" --from mywallet5 --chain-id constantine-3 --fees 500uconst`
...
`archwayd tx gov submit-proposal update-instanciate-config --help --from mywallet5 --node https://rpc.constantine.archway.tech:443 
--chain-id constantine-3 --fees 500uconst --proposal=/root/archway/proposals/testproposal.json`
...
1. `--chain-id` constantine-3 is the flag that specifies the Constantine network
1. `--node https://rpc.constantine.archway.tech:443` is using a RPC endpoint provided by an Archway node to submit the tx in the 
Constantine 
network
...
on the 
terminal. You can either query the tx by using the CLI or by searching the transaction hash using a <a href="https://testnet.archway.explorers.guru/proposals" target="_blank">Constantine blockexplorer</a>.
...
`archwayd tx gov deposit 8  100000uconst --from mywallet4 --node https://rpc.constantine.archway.tech:443`
