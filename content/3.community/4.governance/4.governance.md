...
2. `--fees` is a flat-rate incentive for a validator to process your transaction
   ...
   3. For example, you could use `--fees 500aconst` (which is equal to 0.0001 CONST)
3. `--chain-id` constantine-3 is the flag that specifies the Constantine network
4. `--node https://rpc.constantine.archway.tech:443` is using a RPC endpoint provided by an Archway node to submit the tx in the 
Constantine 
network
