...
1. `--fees` is a flat-rate incentive for a validator to process your transaction
   ...
   1. For example, you could use `--fees 180000000000000000aarch` (which is equal to 0.0001 CONST)
1. `--chain-id` archway-1 is the flag that specifies the mainnet network
1. `--node https://rpc.mainnet.archway.io` is using a RPC endpoint provided by an Archway node to submit the tx in the 
Archway mainnet network.
