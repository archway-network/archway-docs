...
You will execute the following command to create a governance proposal:
...
The following is an example of a `proposal.json` file:
...
Here is an example of the `metadata` that will be added to the `proposal.json` file's `metadata` attribute:
