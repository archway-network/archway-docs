...
The first step is to run a node. See the [Node Installation](../node/install) and [Joining a Network](../node/join-a-network) guides for instructions on how to get your node up and running.
...
Firstly, make sure you have properly [initialized your node](../node/install#initialize-the-node).
...
After having followed the [instructions to join a network](../node/join-a-network) and your running node is synced, you can create a validator by staking tokens. Please make sure to create a validator only once the node is fully synced, so that you avoid being slashed.
...
Now, use the following commant to create your validator:
...
## Next Steps
Great! Now, make sure you [monitor your validator](4.monitoring-validators.md). Monitoring the validator will help to prevent getting slashed, and ensure a higher reliability to your validating operations.
