...
```bash
wget https://github.com/archway-network/archway/releases/download/v1.0.0-rc.3/archwayd_linux_arm64
...
```bash
docker pull ghcr.io/archway-network/archwayd:v1.0.0-rc.3
```
...
```bash
alias archwayd="docker run --rm -it -v ~/.archway:/root/.archway ghcr.io/archway-network/archwayd:v1.0.0-rc.3"
```
...
If you cannot use the pre-built binary or the docker image, you can built the binary yourself.
