...
At this point, your code should be compiling, although you haven't tested whether it works or not. You can deploy the code to the chain each time you make a change, but this is not an efficient use of your time. It's also important to keep the contract intact and tested for future changes.
...
can keep tests and code in the same or separate files.
...
For each test, it's important to mock specific variables such as block time and state. Writing a function for easy setup can make this process more manageable.
...
Good, we now have a test environment initializer. This is a very basic one; you can pass variables to the function and make different modifications. Check out cosmwasm-plus for more options.
