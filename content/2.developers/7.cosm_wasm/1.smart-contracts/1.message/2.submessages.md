...
title: Submessages
description: Messages are used to interact with SDK modules or CW smart contracts
parentSection: Message
---

# Submessages
Messages are used to interact with SDK modules or CW smart contracts. Since messages are executed in a set-and-forget manner, you will not get a response on whether the call was successful or not.

Anyways, getting the result of your call can be very useful in the following cases:
...
To get the result of the message sent from your smart contract, you will need to dispatch a submessage. You can read more on the semantics of submessages and how submessage execution is ordered [here](https://github.com/CosmWasm/cosmwasm/blob/main/SEMANTICS.md#submessages)
...
```
[Here](https://github.com/CosmWasm/cosmwasm/blob/main/packages/std/src/results/submessages.rs) you can find the source code for the ```SubMsg``` struct.
...
```
Note that previously we created the submessage using the ```SubMsg::reply_on_success``` shorthand. However, we can also create a submessage and explicity specifying the reply strategy.
```rust
