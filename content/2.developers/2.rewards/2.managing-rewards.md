...
::highlight-card
```bash
archwayd q rewards rewards-records archway12reqvcenxgv5s7z96pkytzajtl4lf2epyfman2 --node https://rpc.archway.tech:443 --chain-id archway-1 --output json | jq .
```
::
...
```bash
archwayd tx rewards withdraw-rewards --records-limit 100000 --from "${rewards_address}" --node https://rpc.archway.tech:443 --chain-id archway-1
```
...
```bash
archwayd q rewards outstanding-rewards "${rewards_address}" --node https://rpc.archway.tech:443 --chain-id archway-1 --output json | jq .
