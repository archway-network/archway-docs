...
The Cargo.toml file at the root of an Archway project is called its [manifest](https://en.wikipedia.org/wiki/Manifest_file), and contains all of the metadata Cargo needs to compile the smart contract project and its dependencies. Every manifest file consists of one or more sections.
...
To update the Increment [Cargo.toml](https://github.com/archway-network/archway-templates/blob/main/increment/Cargo.toml) to use `cosmwasm-std` version `1.0.0-beta5`, the above file becomes:
