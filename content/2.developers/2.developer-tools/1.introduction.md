...
`arch3.js` is a JavaScript library for interacting with the Archway Protocol. `arch3.js` eliminates the need for working with multiple <a href="https://github.com/cosmos/cosmjs" target="_blank" >`CosmJS` clients</a> by extending their functionalities. It also connects with Archway's unique reward system.
