...
The easiest way to find the list of events linked to a particular transaction is to check the events logs within a block exporer. These event logs will also show fields that can be used as additional filters. For example the following transaction https://www.mintscan.io/archway/tx/A2EFABA14F892C0E559205F5B0B3FB0D1F9533E086BEDAF513954EE2EAC63F69?height=1628545 has the **coin_received** and you can also use the **receiver** field as a filter. The final filter would be **tm.event = 'Tx' AND coin_received.receiver = ''**.
