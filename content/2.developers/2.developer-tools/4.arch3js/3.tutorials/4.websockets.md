...
### Listening to wasm-CallMessage

You can listen to **wasm-CallMessage** by querying the data inside the events within the Cosmos-SDK transaction.

To filter transactions that have a specific event type, like **wasm-CallMessage**, you use a query like:
::highlight-card
...
This tells the system to look for transactions that contain the **wasm-CallMessage** event.

If you want to find transactions related to a particular contract address, you can add an extra filter like:
::highlight-card

```javascript
`wasm-CallMessage._contract_address = '${address}'` 
```

::

This fetches transactions where the **wasm-CallMessage** event has the specified contract address.