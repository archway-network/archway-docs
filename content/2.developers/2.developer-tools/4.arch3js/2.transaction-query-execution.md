...
const client = await ArchwayClient.connect('https://rpc.archway.tech');
...
axios.get(`https://api.archway.tech/cosmwasm/wasm/v1/contract/${contractAddress}/smart`, {
  params: {
    query: query,
  },
})
...
const signingClient = await ArchwayClient.connect('https://rpc.archway.tech');
...
let apiEnpoint = 'https://api.archway.tech';
