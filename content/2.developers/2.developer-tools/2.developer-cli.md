...
| --admin-address [string] | `archway instantiate --admin-address "archway1f395..."` | Address which can perform admin actions on the contract (e.g. "archway1...")             |
...
| Option | Example | Description |
| ----------------------------- | ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| -c, --contract [string] | `archway metadata -c archway1aacn8927t...` | Optional contract address override; defaults to last deployed |
| -f, --from [string] | `archway metadata --from "main"` | Name or address of account to sign transactions |
| --owner-address [string] | `archway metadata --owner-address "archway1f3..."` |  Contract owner address which can change the metadata later on |
| --reward-address [string] | `archway metadata --reward-address "archway1f395p0gg67mm..."` | Address that will receive the rewards |
...
| -c, --contract [string] | `archway query contract-state smart --args '{"entrypoint_name": {}}' -c "archway1zh9gzc..."` | Query a specific contract address                                                                                               |
