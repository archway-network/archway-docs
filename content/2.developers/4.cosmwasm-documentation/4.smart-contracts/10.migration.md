...
In the above example, our `MigrateMsg` has a `verifier` field which contains the updated value for our contract's `verifier` field located in the State. If the contract does not also expose an `UpdateState` or similar `UpdateVerifier` ExecuteMsgs, then this provides the only means of changing the verifier value.
