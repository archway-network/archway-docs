...
In the [Compile Contract](/developers/cosmwasm-documentation/getting-started/compile-contract) article, we generated a wasm binary executable. You can upload the code to the blockchain, and once the process is complete, you can download the bytecode for verification purposes.
...
`mywallet` should be the name of the your wallet created in the [Setting up environment](/developers/cosmwasm-documentation/getting-started/setting-up-environment) article. Execute the following command to store the contract on chain:
...
You can see the list of contracts instantiated using the Code Id above by executing the following command:
...
If the `diff` command returns a blank output, it means that the two files being compared are identical.

### Deploy using archway developer cli

If your contract was created using the `archway new` command, you can deply the contract using the following command:
